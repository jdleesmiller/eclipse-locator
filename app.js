...
function renderWeatherResults(results) {
  ...
  for (const result of results) {
    ...
    if (result.weather) {
      ...
      item.innerHTML = `<button class="weather-result-main" type="button"><strong>${safePlaceLabel}</strong><b class="weather-score">${result.overall.recommendation}</b><span>${result.climateRating} historical cloud cover (${climate.clearOrNearlyClearPct}% clear or nearly clear) · ${terrain.classification} terrain</span></button>
        <small><b>Historical cloud cover:</b> clear or nearly clear on ${climate.clearOrNearlyClearPct}% of comparable occasions · more than one okta ${climate.moreThanOneOktaPct}%<br><b>Terrain:</b> ±0.5° horizon ${terrain.within05DegMaxAngleDeg}° at ${terrain.within05DegMaxDistanceKm} km · Sun ${terrain.sunElevationDeg}° · clearance ${terrain.clearanceDeg >= 0 ? "+" : ""}${terrain.clearanceDeg}°</small>
        ${notesDetail}
