...
The terrain profile samples 100 points from public AWS Terrain Tiles via the same caching proxy used by saved-location comparison. The first 5 km is sampled about every 91 m; the remaining samples cover 5–60 km. The chart can switch between 5, 20 and 60 km views, each with its own vertical scale. After a saved-place comparison, it also shows a labelled one-dimensional low-cloud strip: each colour is the mean forecast percentage across the sampled wedge at that distance, not a cloud-height estimate. A compact distance profile is saved with the comparison so the strip remains available after reload; older saved comparisons fall back to an approximation from their 0/10/25/50 km summaries. Both terrain views use the same observer elevation, nominal 1.7 m eye height and spherical-Earth curvature assumptions, but not atmospheric refraction, buildings or vegetation. Terrain results are planning estimates rather than a visibility guarantee.
