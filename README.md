...
When a selected eclipse is within the short forecast horizon and the observer is in Spain, the app provides AEMET HARMONIE-AROME total/low/high-cloud and cloud-base overlays. Outside that coverage it instead shows an ERA5 historical outlook: the frequency of WMO-defined bright sunshine at the eclipse-time hour during a ±14-day seasonal window, using 2001–2025 for planning and disclosing the 1991–2020 standard normal in details. The opening planning screen keeps locally saved places grouped by eclipse, with editable names and notes. Saved groups can be compared on demand with forecast or historical sunshine as available plus global terrain analysis. See [WEATHER.md](WEATHER.md) for source discovery, architecture, metrics and limitations.
...
The terrain profile samples 100 points from public AWS Terrain Tiles via the same caching proxy used by saved-location comparison. The first 5 km is sampled about every 91 m; the remaining samples cover 5–60 km. The chart can switch between 5, 20 and 60 km views, each with its own vertical scale. Forecast cloud sampling uses a curvature-aware distance derived from solar elevation and nominal low/middle/high layer tops at 3, 8 and 15 km, capped at 150 km; it is no longer fixed at 60 km. Opening **Sightline profile** lazily requests the low-layer portion of a numeric seven-ray cloud corridor for the displayed place; this is separate from the visual WMS map tiles. Both terrain views use the same observer elevation, nominal 1.7 m eye height and spherical-Earth curvature assumptions, but not atmospheric refraction, buildings or vegetation. Terrain results are planning estimates rather than a visibility guarantee.
