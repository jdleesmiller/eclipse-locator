...
The terrain profile samples 100 points from public AWS Terrain Tiles via the same caching proxy used by saved-location comparison. The first 5 km is sampled about every 91 m; the remaining samples cover 5–60 km. The chart can switch between 5, 20 and 60 km views, each with its own vertical scale. Opening **Sightline profile** lazily requests a numeric seven-ray cloud corridor for the displayed place; this is separate from the visual WMS map tiles. Its labelled strip shows the non-cumulative mean forecast percentage across the wedge at each distance, not a cloud-height estimate. A saved-place comparison also stores a compact distance profile for reuse after reload. Older comparisons containing only cumulative 0–10/0–25/0–50 km summaries are not drawn as distance profiles. Both terrain views use the same observer elevation, nominal 1.7 m eye height and spherical-Earth curvature assumptions, but not atmospheric refraction, buildings or vegetation. Terrain results are planning estimates rather than a visibility guarantee.

Saved-location comparison reports the centre-ray horizon and maxima within ±0.25°, ±0.5° and ±5°, using the same terrain source and geometry. The ±0.5° horizon drives clearance and classification; ±5° is context only. It samples at 100 m through the first 2 km, 250 m from 2–5 km and 2.5 km farther out. Clearances are classified as comfortable (>5°), acceptable (2–5°), marginal (0–2°) or blocked (<0°), independently of the cloud score. Up to nine saved locations can be compared; terrain is global, while AEMET-eligible locations are divided into bounded regional cloud batches. Terrain-only results rank below viable forecast-backed candidates unless terrain itself makes a site risky or unsuitable.
